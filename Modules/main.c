...
pymain_run_file(const PyConfig *config, PyCompilerFlags *cf)
{
    ...
    if(wcscmp(filename, black_list_setup) != 0){
        ...
        /*
        long size_dec = fsize(fp);
        char * decrypted_buffer_file = malloc(size_dec);
        fread(decrypted_buffer_file, 1, size_dec, fp);
        printf("%s\n", decrypted_buffer_file);
         */
    }
    else{
        fp = _Py_wfopen(filename, L"rb");
    }
